...
最終更新: 2026-05-19 (★ F-trial-run-candidate-a-reverify 完了、Phase A.5-3a-verify ゲート完了後の **13 つ目のバッチ (1-J)**。候補A cls-6889e9e1c7ac の B-3' 改修後本番完全再現での再確認。CP-1 で (1) 候補A 不在 (完全新規 RSS batch 20260518_111201 のため母集団に不在、F-1/F-2 で落ちたのではない)、(2) 動画化 Slot-1 台本が fallback テンプレ (Gemini 503 多発、`llm_error:RemoteProtocolError`) 判明 → カズヤ判断 = **選択肢3 (Task C/D のみ実施、Task E/CP-2 スキップ)**。B-3' 改修の構造的効果は **3 連続試運転で確定** = has_jp_coverage True 比率単調減少 (post-tune 5/11 **3T/0F** [bypass] → post-llm-extraction 5/16 **1T/2F** [B-3' 配線] → candidate-a-reverify 5/18 **0T/3F** [WL マッチ 0 件で誤陽性 bare-domain マッチ自体が発生せず])。本バッチ目的『候補A の B-3' 改修後本番再確認』は候補A 個別 Slot 再判定ではなく『改修の構造的効果が本番で安定して現れるか』であり 3 連続データで **達成済み**。候補A は **perspective_gap framing で維持** (機械判定 ≠ 事実、perspective_gap は F-wl-hit-quality-audit 2026-05-14 で WebSearch 独立検証済、機械不在は覆さない)。防衛機構 5 層全機能 (異常なし、即停止条件非該当)。新規残課題: F-gemini-503-stability-audit (F-17 候補から昇格、緊急度 高、着手条件達成) + F-periodic-health-check (緊急度 高、Phase A.5-3d 前提)。axis_5 採点は Phase A.5-3b 第一作起案バッチに移送。`src/ tests/ configs/ scripts/ CLAUDE.md` 0 行変更、`docs/` + `data/output/` のみ更新、baseline **1417 passed** 維持、不変原則 1-5 全遵守)
...
効果を 3 連続試運転で確定**。candidate-a-reverify 試運転 (batch_id
20260518_111201) は候補A 不在 (完全新規 RSS batch) + Slot-1 台本 fallback
(Gemini 503 多発) だったが、has_jp_coverage True 比率が 3 連続で単調減少
(3T/0F → 1T/2F → 0T/3F) し、本バッチ目的 (改修の構造的効果確認) は達成。
候補A は perspective_gap framing で維持 (機械判定 ≠ 事実)。Phase A.5-3b
第一作着手 OK (前提最終確定)。
...
★★★ **2026-05-19 (F-trial-run-candidate-a-reverify) 再評価**: 上記乖離は
不変。**B-3' は legacy verify() に配線済**のため LLM judgement bypass 是正は
本配線群とは直交して本番反映済 (3 連続試運転で構造的効果確定)。本番配線判断
バッチ群 3 件は引き続き FUTURE_WORK 緊急度 高に並走待機。
...
- **main HEAD コミット**: `3c964c7` (Merge branch 'feature/F-image-prompt-spec')。F-trial-run-candidate-a-reverify は feature ブランチ `feature/F-trial-run-candidate-a-reverify` で Task A-G 完了、本完了レポート提示後にカズヤ承認 → commit/merge 実行 (Task H)
- **直近 5 件のログ (main)**:
  ```
  ...
  ```
- **baseline テスト数**: **1417 passed** (F-trial-run-candidate-a-reverify は試運転 + docs のみで `src/ tests/ configs/ scripts/ CLAUDE.md` 0 行変更、baseline 完全不変)

## 2. 現在のフェーズ

- **Phase**: Phase A.5-3a-verify **完了** (2026-05-07、ゲート完了後 13 バッチ目が本バッチ 1-J)
- **進行中バッチ**: なし (F-trial-run-candidate-a-reverify 完了直後、Task G 完了レポート提示 → カズヤ承認待ち → commit/merge Task H)
- **次バッチ候補と推奨** (★ F-trial-run-candidate-a-reverify / 2026-05-19 更新):
  - **1st: Phase A.5-3b 第一作起案** ★★★ 最有力 (緊急度 高、ADR-0001/0002/0003 + `schema_extension_design.md` 前提。前提は本バッチで最終確定。候補A cls-6889e9e1c7ac を **手動 event 固定 + 実台本生成** + perspective_gap framing + axis_5 採点 (本バッチから移送)。models.py に VideoImage/VideoEvent Optional 追加 + video_payload_writer.py 最小改変 + Remotion D-minimal 構築)
  - **2nd: F-gemini-503-stability-audit** ★ (緊急度 高、F-17 候補から昇格、着手条件「503 多発確認」達成。`src/llm/factory.py` + `retry.py` のリトライ間隔動的調整 / サーキットブレーカー / Slot 別 fallback 戦略見直し。Phase A.5-3b と並走推奨 = 第一作実台本が 503 で fallback 落ちるリスク低減)
  - **3rd: F-periodic-health-check** ★ (緊急度 高、新規。production 全工程の定期ヘルスチェック。Phase A.5-3d cron 完全自動投稿の前提)
...
- **推奨フロー**:
  - commit/merge (本完了レポート提示 → カズヤ承認後)
    → Phase A.5-3b 第一作起案 (候補A cls-6889e9e1c7ac 手動固定 + perspective_gap framing + axis_5 採点、ADR + schema 前提)
    → 並走: F-gemini-503-stability-audit + F-periodic-health-check + F-grounding-determinism-audit + 高リスク事実検証ワークフロー + 本番配線判断バッチ群
...
- **★ Phase A.5-3b 第一作着手前の追加確認事項** (カズヤ指示、2026-05-19 更新):
  ...
  5. ★ F-gemini-503-stability-audit を Phase A.5-3b 着手前 or 並走で検討 (第一作実台本の 503 fallback リスク低減)

### Phase A.5-3a-verify ロードマップ (★ F-trial-run-candidate-a-reverify / 2026-05-19 更新版)

**ゲート完了**: 1-A〜1-D''' 全段階完了で Phase A.5-3a-verify ゲート完了 (2026-05-07)。
本バッチはゲート完了後の **13 つ目のバッチ (1-J)**。
...
| 1-K | F-image-prompt-spec | ✅ 完了 (2026-05-18) | 3 AI 三角測量 D-minimal 仕様を ADR 3 件 + video_payload schema 拡張設計として正典化 (設計のみ) |
| **1-J** | **F-trial-run-candidate-a-reverify** | ✅ **完了 (2026-05-19)** | **ゲート完了後 13 つ目**。候補A B-3' 改修後本番再確認。候補A 不在 + Slot-1 fallback だったが B-3' 構造的効果を 3 連続試運転 (3T/0F → 1T/2F → 0T/3F) で確定 = 目的達成。候補A perspective_gap 維持、Phase A.5-3b 前提最終確定。F-17 候補昇格 (F-gemini-503-stability-audit)。`src/ tests/ configs/ scripts/ CLAUDE.md` 0 変更、baseline 1417 維持 |
| 1-L | Phase A.5-3b 第一作起案 | ★ 緊急度 高 (ADR + schema 前提、前提最終確定済) | 1-J 完了で着手可、候補A 手動固定 + perspective_gap framing + axis_5 採点 |
| 1-M | F-gemini-503-stability-audit | ★ 緊急度 高 (着手条件達成) | リトライ間隔動的調整 / サーキットブレーカー / Slot 別 fallback 戦略、Phase A.5-3b 並走推奨 |
| 1-N | F-periodic-health-check | ★ 緊急度 高 (新規) | production 全工程定期ヘルスチェック、Phase A.5-3d 前提 |
| 1-O | F-grounding-determinism-audit | ★ 緊急度 中 | broad Grounding API の WL ドメイン返却率 run 間分散の集約戦略検討 |
| 1-P | 本番配線判断バッチ群 (3 件) | ★ 並走候補 | verify_two_stage / particular_angle_metadata+sontaku_signals / F-stream-2-filter-design |
...
| 試運転 | バッチ | 動画化率 | 主要観察 |
|---|---|---|---|
| **2026-05-18** | **F-trial-run-candidate-a-reverify** | **1/3 動画化 (Slot-1 cls-f47e9ffde77d, ★ fallback script) + 3 articles** | ★ 候補A cls-6889e9e1c7ac 不在 (完全新規 RSS batch、機械判定 ≠ 事実)。3 Slot 全件 has_jp_coverage=False かつ WL マッチ 0 件 (tier=None) = bare-domain bypass 構造的に発生せず。has_jp True 比率 3 連続単調減少 (5/11 3T/0F → 5/16 1T/2F → 5/18 0T/3F)。Slot-1 台本 fallback (Gemini 503 多発、`llm_error:RemoteProtocolError`)。防衛機構 5 層全機能。axis_5 は Phase A.5-3b に移送。F-17 候補昇格。 |
| 2026-05-18 | F-image-prompt-spec | 試運転なし (docs バッチ) | 3 AI 三角測量 D-minimal 仕様を ADR 3 件 + schema 設計に正典化。 |
| 2026-05-16 | F-trial-run-post-llm-extraction | 1/3 動画化 (Slot-1 cls-e2429c77f48e) + 3 articles | ★★★ B-3' が production verify() に配線・本番で安全装置初発火 (Slot-3 WL tier_2 + no_match → False)。has_jp 分布 3/3 True → 1 True / 2 False に反転。axis_5 候補B=15/25。第一作題材確定 = 候補A perspective_gap。 |
| 2026-05-16 | F-jp-coverage-llm-judgement-extraction | (試運転なし、ゴールデン 23 件再測定) | LLM judgement bypass 根本治療。WL マッチ条件下サブセットで Recall 1.0000 / Precision 0.8889 / FN=0。 |
...
| 2026-05-11 | F-trial-run-post-tune | 1/3 動画化 + 3 articles | 3 Slot 全件 has_jp_coverage=True (bare-domain bypass)。第一作機械スコア Slot-1 cls-6889e9e1c7ac 最有力。 |

## 4. Hydrangea コンセプト防衛機構の現状 (5 層)

> ★ F-trial-run-candidate-a-reverify (2026-05-19) 本番試運転 (batch_id
> 20260518_111201) で全 5 層の挙動を再確認。**異常挙動なし、即停止条件
> 非該当**。Slot-1 fallback は script_writer 設計上の安全網であり防衛機構
> 異常ではない (Gemini 503 多発 run が背景、F-17 候補昇格の根拠)。

| 層 | バッチ | 場所 | 役割 | 状態 |
|---|---|---|---|---|
| F-1 | F-1 / F-1.5 | EditorialMissionFilter | 編集ミッション適合度で score 算出 (>= 45.0 で通過) | ✅ 稼働中 (本 run 351→19 通過、prescore max 65.3/mean 22.48) |
| F-2 | F-2 / F-5 | FlagshipGate / EliteJudge | 海外発の重要ニュースを優先 | ✅ 稼働中 (Gate3 10評価→採用7/棄却3、GeminiJudge 1→blind_spot_global、CoherenceGate 1 PASSED、Blocked 0) |
| F-13.B | F-13.B / … / F-jp-coverage-llm-judgement-extraction / F-trial-run-post-llm-extraction / F-trial-run-candidate-a-reverify | JpCoverageVerifier (WL 30 ドメイン階層判定 + LLM judgement 抽出 B-3') | JP 報道カバレッジを WL + LLM judgement で検証 | ✅ **B-3' 改修の構造的効果を 3 連続試運転で確定** (has_jp True 比率 3T/0F → 1T/2F → 0T/3F)。本 run は 3 invocations 全 False / WL マッチ 0 件 / 安全装置 0 発火 (WL 0 で発火条件外、安全装置本番発火は post-llm-extraction で 1 件実証済)。verify_two_stage 系統機械判別は依然本番未配線 (B-3' とは直交) |
| F-5 | F-5 | FlagshipGate 下流救済 | 上流ガードを通過した候補の最終整合 | ✅ 稼働中 (本 run 0 件発火 = 入力依存、異常なし) |
| F-13 (隠れ層) | F-13 / F-doc-cleanup | script_writer.py quality_floor_miss bypass | analysis_result 等が成立すれば appraisal の [抑制] を上書き | ✅ 稼働中 (本 run 0 件発火 = quality floor ブロック自体が発生せず bypass 不要、post-llm-extraction と同挙動、設計通り) |
...
- `src/llm/factory.py` / `src/llm/retry.py` (★ F-gemini-503-stability-audit で改修対象、不変原則対象外)
...
- **「機械判定は事実の代替ではない」** (★ F-trial-run-candidate-a-reverify
  2026-05-19) — 候補A が機械的に拾われなくても perspective_gap 確定
  (WebSearch 独立検証済) は覆らない。候補A に固執せず、目的 (B-3' 構造的
  効果の観察) は 3 連続データで達成済みと判断
- **「中間が良い」** — シニカル一辺倒でも生活実感一辺倒でもなく両立。
  第一作 (候補A) は punchline メディア断定回避を framing 指針化 (ADR-0003)
- **「考え方で制御」** — NG リスト方式は廃止、原則ベースのプロンプト
- **「対症療法じゃなくて根本治療」** + **「動くものを壊さない」** + **「将来に
  負債を残さない」** — ★ F-trial-run-candidate-a-reverify では Slot-1
  fallback を即停止せず観察として記録、Gemini 503 を F-17 候補昇格 +
  F-periodic-health-check 新規起案で構造的に対処 (対症療法回避)
...
- ★ **F-trial-run-candidate-a-reverify REPORT + 分析** → `docs/runs/F-trial-run-candidate-a-reverify/REPORT.md` + `f13b_comparison.json` + `candidate_a_analysis.json` + `trial_run_summary.json`
- F-image-prompt-spec REPORT + 設計 → `docs/runs/F-image-prompt-spec/REPORT.md` + `schema_extension_design.md`
- F-trial-run-post-llm-extraction REPORT → `docs/runs/F-trial-run-post-llm-extraction/REPORT.md`
- F-jp-coverage-llm-judgement-extraction REPORT + 設計 → `docs/runs/F-jp-coverage-llm-judgement-extraction/REPORT.md` + `design_spec_v2.md` (B-3')

---

*このドキュメントは F-state-protocol (2026-05-01) で導入。Claude Code が
バッチ完了時に全置換更新する運用 (BATCH_PROTOCOL.md Task 5)。
F-trial-run-candidate-a-reverify (2026-05-19) は **ゲート完了後の 13 つ目の
バッチ (1-J)**。候補A cls-6889e9e1c7ac の B-3' 改修後本番完全再現での再確認。
CP-1 で候補A 不在 (完全新規 RSS batch) + 動画化 Slot-1 台本 fallback
(Gemini 503 多発) 判明 → カズヤ判断 = 選択肢3 (Task C/D のみ、Task E/CP-2
スキップ)。B-3' 改修の構造的効果は 3 連続試運転で確定 (has_jp True 比率
3T/0F → 1T/2F → 0T/3F の単調減少)、本バッチ目的 (改修の構造的効果が本番で
安定して現れるか) は達成。候補A は perspective_gap framing で維持 (機械判定
≠ 事実)。防衛機構 5 層全機能 (異常なし、即停止条件非該当)。新規残課題
F-gemini-503-stability-audit (F-17 候補昇格、緊急度 高) + F-periodic-health-
check (緊急度 高、Phase A.5-3d 前提)。axis_5 採点は Phase A.5-3b 第一作起案
バッチに移送。`src/ tests/ configs/ scripts/ CLAUDE.md` 0 行変更、`docs/` +
`data/output/` のみ更新、baseline **1417 passed** 維持、不変原則 1-5 全遵守。
次バッチ候補 = Phase A.5-3b 第一作起案 (★ 最有力、前提最終確定済) →
F-gemini-503-stability-audit + F-periodic-health-check 並走 →
F-grounding-determinism-audit + 高リスク事実検証ワークフロー + 本番配線判断
バッチ群。過去の経緯は DECISION_LOG.md / FUTURE_WORK.md /
