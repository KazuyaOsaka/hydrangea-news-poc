...
### F-13 隠れ層 (script_writer.py の quality_floor_miss bypass)

> このセクションは F-doc-cleanup (2026-05-03) で追加された遡及記録。詳細経緯は
> `docs/DECISION_LOG.md` の以下 2 エントリを参照:
> - `2026-05-03: F-doc-cleanup — F-13 隠れ層の正式昇格 (遡及記録)`
> - `2026-05-03: F-doc-cleanup — F-13 (rescue 廃止前提整備) 本体 (遡及記録)`

#### 位置付け

`src/generation/script_writer.py:951-985` に存在する `quality_floor_miss` bypass
ロジックは、Hydrangea コンセプト防衛機構の **5 層目** (隠れ層) として位置付ける。
Filter (F-1 EditorialMissionFilter) / Gate (F-2 FlagshipGate) /
Verifier (F-13.B JpCoverageVerifier) / FinalSelection (F-5 下流救済) と並ぶ
独立した安全網である。

#### 発動経路

`triage_result.appraisal_cautions` が `[抑制]` で始まる候補に対して、
以下のいずれか 1 つ以上を満たせば抑制を上書きして通過させる:

1. `editorial_mission_score >= MISSION_SCORE_THRESHOLD`
   (F-1 通過レベルの編集ミッション適合度)
2. `judge_result.publishability_class in {blind_spot_global, linked_jp_global}`
   (F-2 通過判定: Hydrangea コンセプト本丸記事と認められた)
3. `analysis_result is not None`
   (AnalysisLayer で多角的分析が成立、独立した証拠系統)

bypass 発動時は `[F-13] quality_floor_miss bypass` の WARNING ログで可視化。
3 条件をいずれも満たさない場合は `ValueError` で動画化を停止。

#### 防衛機構との関係

- **F-1 EditorialMissionFilter (1 層目)**: 編集ミッション適合度 (7 軸 100 点満点)
  で score >= 45.0 を通過判定。Step 1 prescore (静的) と Step 2 LLM 採点で構成
- **F-2 FlagshipGate (2 層目)**: Hydrangea コンセプト整合
  (海外発の重要ニュースを優先)。publishability_class による判定
- **F-13.B JpCoverageVerifier (3 層目)**: 27+ ドメイン WL で日本語 Web 検証。
  rescue 完全廃止後の唯一の JP 報道判定経路
- **F-5 FinalSelection 下流救済 (4 層目)**: publishability_class が
  investigate_more / insufficient_evidence でも、blind_spot >= 5.0 OR
  ijai >= 5.0 + editorial_mission_score >= 45.0 で flagship 認定
- **F-13 隠れ層 (5 層目、本セクション)**: script 生成段階で残った [抑制] マークを、
  上流ガードのいずれかが「本丸記事」と認めていれば上書きして通過

#### 設計上の判断

- **AnalysisLayer 条件 (3) の独立性**: AnalysisLayer は Filter/Gate/Verifier から
  独立した分析系統。多角的分析が成立した = 編集ミッションの「届けるべき記事」と
  認める根拠が独立して揃っている、と解釈する
- **WARNING ログによる可視化**: bypass 発動を見えなくすると「なぜ動画化された
  のか」が後から追えなくなるため、必ず WARNING ログで event_id +
  editorial_mission_score + judge_class + analysis_result の有無を記録
- **不変原則 2 との整合**: 本ロジックは script_writer.py の **新ルート**
  (`generate_script_with_analysis`) から呼ばれる経路で稼働。既存ルート
  (`write_script` / `_PROMPT_TEMPLATE` / `_build_script_from_llm`) は不変
