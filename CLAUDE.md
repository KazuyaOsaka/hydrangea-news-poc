...
新規バッチ着手時は以下を **この順序で** 必ず参照すること：

1. **`docs/CURRENT_STATE.md`** ★最優先
   — プロジェクトの「今この瞬間のスナップショット」。
   main HEAD / Phase / 不変原則 / 次バッチ候補等の最新状態。
   バッチ完了時に Claude Code が全置換更新する (BATCH_PROTOCOL Task 5)。
2. **`docs/BATCH_PROTOCOL.md`** — 全バッチ共通の必須タスク (Task 1-5) と
   不変原則 5 つの正本。
...
6. **`docs/ANALYSIS_LAYER_DESIGN_v1.1.md`** — 分析レイヤー設計書 (実装の正典)。
7. **`docs/ARCHITECTURE.md`** / **`docs/TECH_DEBT.md`** /
   **`docs/REFACTORING_PLAN.md`** — 必要に応じて参照。
8. **`docs/CLAUDE_CODE_INSTRUCTIONS.md`** — バッチ分割された実装指示書
   (分析レイヤー実装期の遺産、現運用は BATCH_PROTOCOL ベース)。
