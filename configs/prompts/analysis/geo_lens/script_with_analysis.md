...
★ 事実整合の原則（coverage claim = 報道状態の主張）:
自分の stream_classification が示す報道状態に **反する未報道断定をしない**。これは
具体的な言い回しの強制ではなく、「事実に反するな」という原則です（語の選択は引き続き
あなたの知性に委ねます）。
- **perspective_gap**（事件は報道済 + 特定角度のみ未報道）/ **framing_inversion**
  （報道済 + 解釈差）の場合、事件本体は日本でも報道済みである事実を踏まえ、「この事件は
  日本では一切報道されない」「日本の主流メディアは黙殺した」のように **事件そのものが
  未報道だと読める silence 絶対表現を使わない**。触れてよいのは「事件は報じられたが、
  ◯◯という角度 / 構造には触れられていない」（perspective_gap）/「日本は××と捉えたが、
  海外は△△と批判している」（framing_inversion）まで。
- **silence_gap**（完全な情報空白）の場合のみ「日本では報じられなかった」「黙殺されてきた」
  が事実として成立する。
- メタデータが "(none)" の場合は報道状態が確定しないため、未報道の絶対断定は避け、
  insights が裏づける範囲で語る。
