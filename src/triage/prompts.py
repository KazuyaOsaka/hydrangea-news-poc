...
- 【重要・判定拡張ルール】日本企業や日本人が直接関与していなくても、以下に該当する場合は高得点（5点以上）を与えること：
  (1) グローバルなパワーバランスやルールの変更（欧米の対立・BRICS台頭・多極化など）
  (2) 世界経済やサプライチェーンの前提を覆すマクロ事象（エネルギー・金融・貿易構造の転換）
  (3) 日本のビジネスパーソンが「教養」や「リスクシナリオ」として知っておくべき強烈なパラダイムシフト
- 【低得点条件】他国の単なる国内選挙（世界的波及効果なし）・局地的事故・芸能ゴシップなど、日本の読者が完全に「対岸の火事」とスルーするローカルニュースは徹底的に低得点（2点以下）とすること
...
- 他国の単なるローカルニュース（局地的事故・国内政局・芸能ゴシップ）を、日本への波及なしに高く評価すること
...
- 単なる有名人ゴシップ
- 海外の反応まとめで終わるもの
- 日本への直接・間接いずれの波及も見込めないローカルニュース（世界的パラダイムシフトや地政学的重大性がない場合のみ）
- すでに日本でも大量に報じられている話
- 背景を語る余地が薄いもの
